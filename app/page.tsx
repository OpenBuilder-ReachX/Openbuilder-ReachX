  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <Link href="/candidates" className="glass-panel block rounded-xl p-6 shadow-sm transition-all hover:scale-[1.02] hover:shadow-md">
          <div className="text-sm font-medium text-neutral-600 dark:text-neutral-400">Total Candidates</div>
          <div className="mt-2 text-3xl font-bold font-tracking-tight">{candidateCount || 0}</div>
        </Link>

        <Link href="/clients" className="glass-panel block rounded-xl p-6 shadow-sm transition-all hover:scale-[1.02] hover:shadow-md">
          <div className="text-sm font-medium text-neutral-600 dark:text-neutral-400">Active Clients</div>
          <div className="mt-2 text-3xl font-bold font-tracking-tight">{clientCount || 0}</div>
        </Link>

        <Link href="/jobs" className="glass-panel block rounded-xl p-6 shadow-sm transition-all hover:scale-[1.02] hover:shadow-md">
          <div className="text-sm font-medium text-neutral-600 dark:text-neutral-400">Open Job Orders</div>
          <div className="mt-2 text-3xl font-bold text-blue-600 font-tracking-tight">{jobCount || 0}</div>
        </Link>

        <Link href="/candidates?status=Green" className="glass-panel block rounded-xl border-l-4 border-l-green-500 bg-white/50 p-6 shadow-sm transition-all hover:scale-[1.02] hover:shadow-md">
          <div className="text-sm font-medium text-green-700 dark:text-green-400">Ready to Deploy</div>
          <div className="mt-2 text-3xl font-bold text-green-700 dark:text-green-400 font-tracking-tight">{greenCount || 0}</div>
        </Link>
      </div>
      ...
      <section>
        ...
        <div className="mt-4 flex flex-wrap gap-4">
          <Link
            href="/jobs"
            className="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-500"
          >
            Create Job Order
          </Link>
          <Link
